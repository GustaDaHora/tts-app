export function mergeAudioBuffers(buffers: AudioBuffer[], ctx: AudioContext): AudioBuffer | null {
    ...
    const outputString = buffers.reduce((acc, curr) => acc + curr.length, 0);
    const numberOfChannels = buffers[0].numberOfChannels;
    const sampleRate = buffers[0].sampleRate;
    
    const outputBuffer = ctx.createBuffer(numberOfChannels, outputString, sampleRate);
...
function floatTo32BitPCM(output: DataView, offset: number, input: Float32Array) {
    for (let i = 0; i < input.length; i++, offset += 4) {
      const s = Math.max(-1, Math.min(1, input[i]));
      // 32-bit integer PCM scaling
       output.setInt32(offset, s < 0 ? s * 2147483648 : s * 2147483647, true);
    }
  }
